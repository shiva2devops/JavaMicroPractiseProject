...
public class AccountControllerIntegrationTest {
    ...
    @Test
    public void testUpdateAccountController() throws Exception{
        String secret= "Basic " + Base64Utils.encodeToString(("manager1"+":"+"password").getBytes());
        Account expectedAccount= new Account("123",20000,"account1");
        Mockito.when(repositoryService.updateAccount(anyString(),anyInt())).thenReturn(expectedAccount);
        ResultActions resultActions= mockMvc.perform(get("/update/account").header(HttpHeaders.AUTHORIZATION,secret).header("2342",4535));
        resultActions.andExpect(status().isOk());
        String result= resultActions.andReturn().getResponse().getContentAsString();
        Account actualAccount= asJsonStringToObject(result);
        Assert.assertEquals(actualAccount.getId(),expectedAccount.getId());
